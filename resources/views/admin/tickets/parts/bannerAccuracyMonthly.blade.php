   @php
   $tickets = App\Ticket::where('created_at','>', \Carbon\Carbon::now()->subMonths(1))->where('status', '=', '1')->count() * 100;
   $good = \App\Ticket::where('created_at','>', \Carbon\Carbon::now()->subMonths(1))->where('status', '=', '1')->count() + \App\Ticket::where('created_at','>', \Carbon\Carbon::now()->subMonths(1))->where('status', '=', '2')->count();
  if($good == 0) {
    $acc = 0;
  } else {
