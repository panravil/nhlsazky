                ...
                <div class="px-3 pb-3">
                    <div class="">
                        <table class="table table-striped table-hover table-bordered">
                            ...
                            <tr>
                                <th colspan="2"><a href="{{ route('accountEdit') }}"
                                                   class="btn btn-sm btn-block tlacitko_live_nastaveni text-white">UPRAVIT
                                        EMAIL</a></th>
                            </tr>
                            <tr>
                                <th>Registrovan:</th>
                    ...
                    <div class="btn-group-sm">
                        ...
                        <a class="btn btn mt-3 btn-block btn-secondary" href="/odhlasit"><i class="fas fa-sign-out"
                                                                     aria-hidden="true"></i>ODHLÁSIT</a>
                </div>

                @if(!Auth::user()->hasVerifiedEmail())
                    <div class="alert alert-danger  d-flex justify-content-between align-items-center" role="alert">
                        Emailová adresa ještě nebyla ověřena. Notifikační emaily tak nebude možné doručit. <a
                            href="{{ route('verification.notice') }}"
                            class="btn btn-primary btn-block text-uppercase"><span
                                class="text-white">Ověřit EMAIL</span></a>
                    </div>
                @endif
            </div>
                    ...
                    @foreach(\Illuminate\Support\Facades\Auth::user()->subscriptionsValid as $sub)
                        @if(\Carbon\Carbon::today()->diffInDays($sub->to) < 3)
                            <div class="alert alert-danger" role="alert">
                                <h5 class="alert-heading p-1"><b>{{ $sub->package->title }}</b>: členství brzo
                                    vypší.
                                </h5>
                                ...
                                @if($sub->package->show == 1)
                                    <a href="#buy-1" data-toggle="modal" data-target="#buy-{{ $sub->package->id }}"
                                       class="btn btn-block btn-primary shadow" style="color: white;">PRODLOUŽIT
                                        ČLENSTVÍ</a>
                                @endif
                            </div>
                        @endif
